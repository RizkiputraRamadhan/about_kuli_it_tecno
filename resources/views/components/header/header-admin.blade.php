                ...
                <li class="nav-item dropdown">
                    <a class="nav-link d-flex align-items-center" href="javascript:void(0)" id="drop2" data-bs-toggle="dropdown" aria-expanded="false">
                        @if (auth()->user()->roles == 'ADMIN')
                            <img src="{{ asset('assets/admin.jpg') }}" alt="" width="35" height="35" class="rounded-circle">
                            @php
                                $name = auth()->user()->name;
                                $nameParts = explode(' ', strtolower($name));
                                $formattedName = ucwords($nameParts[0]) . (isset($nameParts[1]) ? ' ' . ucwords($nameParts[1]) : '');
                            @endphp
                            <small class="p-1 mt-1 d-none d-md-inline" style="font-size: 15px">{{ $formattedName }}</small>
                        @else
                            @php
                                $name = auth()->user()->name;
                                $nameParts = explode(' ', strtolower($name));
                                $formattedName = ucwords($nameParts[0]) . (isset($nameParts[1]) ? ' ' . ucwords($nameParts[1]) : '');
                            @endphp
                            <img src="{{ asset('assets/user.png') }}" alt="" width="35" height="35" class="rounded-circle">
                            <small class="p-1 mt-1 d-none d-md-inline" style="font-size: 15px">{{ $formattedName }}</small>
                        @endif
                    </a>
                        ...
                        <div class="message-body">
                            @if (auth()->user()->roles == 'ADMIN')
                                <a href="javascript:void(0)" class="d-flex align-items-center gap-2 dropdown-item"
                                    ...
                                    <p class="mb-0 fs-3">My Accounts</p>
                                </a>
                            @endif
            ...
            <div class="modal-content">
                ...
                <form action="" method="POST">
                    ...
                    <input type="hidden" class="form-control" id="roles" name="roles"
                        value="{{ auth()->user()->roles }}" required>
                    @csrf
